...
export async function afficherGroupes(user: Utilisateur) {
  ...
  while (continuer) {
    ...
    const groupesRejoints = groupes.filter(
      (g) => g.membreId?.includes(user.id) && g.chefDeGroupe !== user.id
    );
    ...
    const choixGroupes = [
      ...(mesGroupes.length > 0
        ? [
            ...
            ...mesGroupes.map((g) => ({
              name: `${chalk.bold.green(g.nom)} - ${chalk.gray(
                g.description
              )} ${chalk.blue("(Créateur)")}`,
              value: g.id,
            })),
...
async function menuGroupe(user: Utilisateur, groupeId: number) {
  ...
  const estMembreRejoint =
    groupeSelectionne.membreId?.includes(user.id) && !estCreateur;
      ...
      boxen(
        chalk.bold.cyan(`${groupeSelectionne.nom.toUpperCase()}`) +
          ...
          chalk.blue(
            `Créé par: ${
              loadData().users.find(
                (u) => u.id === groupeSelectionne.chefDeGroupe
              )?.nom || "Inconnu"
            }`
          ),
...
async function afficherMonSoldePerso(userId: number, groupeId: number) {
  ...
  const depensesGroupe = data.depenses.filter(
    (d: any) => d.groupeId === groupeId
  );
  ...
  const paiementsMembre = data.Payer.filter(
    (p: Payer) =>
      p.membreId === userId &&
      depensesGroupe.some((d: any) => d.id === p.depenseId)
  );
  totalPaye = paiementsMembre.reduce(
    (sum: number, p: Payer) => sum + p.sold,
    0
  );
  ...
  console.log(
    ` Membre: ${chalk.yellow(utilisateur.nom)} (${chalk.blue(
      utilisateur.email
    )})`
  );
...
async function payerCeQueJeDois(
  userId: number,
  groupeId: number,
  methode: "espèces" | "mobile_money" | "carte_bancaire" = "espèces"
) {
  const data = loadData();
  const groupe = data.groups.find((g: any) => g.id === groupeId);
  if (!groupe) {
  ...
  depensesGroupe.forEach((depense: any) => {
    if (depense.membreId.includes(userId)) {
      ...
      if (resteAPayer > 0) {
        ...
        console.log(
          chalk.green(
            ` Paiement de ${resteAPayer} FCFA validé pour dépense ID: ${depense.id}`
          )
        );
      }
    }
...
async function payerPartiellement(
  userId: number,
  groupeId: number,
  montantPaye: number,
  methode: "espèces" | "mobile_money" | "carte_bancaire" = "espèces"
  ...
  if (montantPaye > montantDu) {
    console.log(
      chalk.red(
        ` Vous essayez de payer plus que ce que vous devez (${montantDu} FCFA)`
      )
    );
    return;
  }
  ...
  for (const depense of depensesGroupe) {
    if (depense.membreId.includes(userId) && resteAPayer > 0) {
      ...
      const aPayerPourCetteDepense = Math.min(
        partIndividuelle - dejaPaye,
        resteAPayer
      );
      ...
      if (aPayerPourCetteDepense > 0) {
        ...
        console.log(
          chalk.green(
            ` Paiement de ${aPayerPourCetteDepense} FCFA validé pour dépense ID: ${depense.id}`
          )
        );
      }
    }
...
async function afficherDepenses(
  ...
  if (depensesAAfficher.length === 0) {
    console.log(
      chalk.yellow(" Vous n'avez pas encore créé de dépenses dans ce groupe")
    );
    return;
  }
  ...
  depensesAAfficher.forEach((d, index) => {
    ...
    console.log(
      `${chalk.bold(`#${index + 1}`)} ${chalk.yellow(d.nom)}\n` +
        ...
        `   Créateur: ${chalk.blue(
          createur?.id === user.id ? "Vous" : createur?.nom || "Inconnu"
        )}\n` +
        chalk.gray("────────────────────────────────────────")
    );
  });
  ...
  const { confirm } = await inquirer.prompt([
    {
      ...
      message: chalk.red.bold(
        " Voulez-vous vraiment supprimer définitivement ce groupe ?"
      ),
      default: false,
    },
  ]);
